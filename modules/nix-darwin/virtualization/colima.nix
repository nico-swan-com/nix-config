...
let
  cfg = config.services.nicoswan.colima;
  ...
  mkListStrOption = description: default:
    mkOption {
      inherit description;
  ...
  mkAttrsOption = description: default:
    mkOption {
      inherit description;
  ...
  kubernetesType = lib.types.submodule {
    options = {
      ...
      kubernetesDisable =
        mkListStrOption "The kubernetes components to disable." [ ];
    };
  };
  ...
  vmConfigType = lib.types.submodule {
    options = {
      ...
      autoActivate =
        mkBoolOption "Auto-activate on the Host for client access." true;
      kubernetes = mkOption {
      ...
      forwardAgent =
        mkBoolOption "Forward the host's SSH agent to the virtual machine."
        false;
      docker = mkAttrsOption
        "Docker daemon configuration that maps directly to daemon.json." { };
      vmType = mkStrOption "Virtual Machine type (qemu, vz)" "qemu";
      rosetta = mkBoolOption
        "Utilise rosetta for amd64 emulation (requires m1 mac and vmType `vz`)"
        false;
      mountType = mkStrOption
        "Volume mount driver for the virtual machine (virtiofs, 9p, sshfs)."
        "sshfs";
      mountInotify =
        mkBoolOption "Propagate inotify file events to the VM." false;
      cpuType = mkStrOption
        "The CPU type for the virtual machine (requires vmType `qemu`)." "host";
      provision = mkOption {
        description = "Custom provision scripts for the virtual machine.";
        type = types.listOf (types.attrsOf types.attrs);
        default = [ ];
      };
      sshConfig = mkBoolOption
  ...
  generateConfigFile = vm:
    pkgs.runCommand "${vm.hostname}.yaml" { } ''
...
in {
  options.services.nicoswan.colima = {
  ...
  config = let
    kubernetesEnable = vm:
      lib.optionalString (vm.kubernetes.enabled == true) " --kubernetes=true";
    kubernetesVersion = vm:
      lib.optionalString (vm.kubernetes.version != null)
      " --kubernetes-version=${vm.kubernetes.version}";
    k3sArgsOptions = vm:
      "[${
        lib.concatStringsSep " "
        (builtins.map (config: "${config}") vm.kubernetes.k3sArgs)
      }]";
    k3sArgs = vm:
      lib.optionalString ((k3sArgsOptions vm) != "[]")
      " --k3s-arg=${kubernetesDisableOptions vm}";
    kubernetesDisableOptions = vm:
      "${lib.concatStringsSep ","
      (builtins.map (config: "${config}") vm.kubernetes.kubernetesDisable)}";
    kubernetesDisable = vm:
      lib.optionalString ((kubernetesDisableOptions vm) != "")
      " --kubernetes-disable=\"${kubernetesDisableOptions vm}\"";
    kubernetes = vm:
      "${kubernetesEnable vm}${kubernetesVersion vm}${k3sArgs vm}${
        kubernetesDisable vm
      }";

    cpu = vm: lib.optionalString (vm.cpu != null) " --cpu=${toString vm.cpu}";
    memory = vm:
      lib.optionalString (vm.memory != null) " --memory=${toString vm.memory}";
    disk = vm:
      lib.optionalString (vm.disk != null) " --disk=${toString vm.disk}";
    arch = vm: lib.optionalString (vm.arch != null) " --arch=${vm.arch}";
    hardware = vm: "${cpu vm}${memory vm}${disk vm}${arch vm}";

    networkAddress = vm:
      lib.optionalString (vm.network.address == true) " --network-address=true";
    hostname = vm:
      lib.optionalString (vm.hostname != null) " --hostname=${vm.hostname}";
    dns = vm:
      " ${
         lib.concatStringsSep " "
         (builtins.map (ip: "--dns=${ip}") vm.network.dns)
       }";
    network = vm: "${hostname vm}${networkAddress vm}${dns vm}";

    startScript = vm:
      pkgs.writeScriptBin "start-${vm.launchd.labelPrefix}-${vm.hostname}.sh" ''
    ...
    launchdScript = vm:
      pkgs.writeScriptBin
      "launchd-${vm.launchd.labelPrefix}-${vm.hostname}.sh" ''
        ...
        start_colima() {
                ...
                ${
                  startScript vm
                }/bin/start-${vm.launchd.labelPrefix}-${vm.hostname}.sh
            #fi
        }
  ...
  in mkIf cfg.enable {
    ...
    home.packages = with pkgs; mkIf (cfg.vms != [ ]) [ colima fswatch ];

    programs.zsh = {
      shellAliases = lib.mkMerge (map (vm: {
        "kube-colima-${vm.hostname}-context" =
          "kubectl config use-context colima-${vm.hostname}";
      }) cfg.vms);

      initContent = lib.mkMerge (map (vm: ''
        ...
        "${vm.launchd.labelPrefix}-${vm.hostname}" = {
          ...
          config = {
            ProgramArguments = [
              ...
              "${
                launchdScript vm
              }/bin/launchd-${vm.launchd.labelPrefix}-${vm.hostname}.sh"
            ];
            Label = "${vm.launchd.labelPrefix}-${vm.hostname}";
            LaunchOnlyOnce = true;
            StandardErrorPath =
              "${config.home.homeDirectory}/Library/Logs/${vm.launchd.labelPrefix}-${vm.hostname}.stderr.log";
            StandardOutPath =
              "${config.home.homeDirectory}/Library/Logs/${vm.launchd.labelPrefix}-${vm.hostname}.stdout.log";
            RunAtLoad = true;
            KeepAlive = false;
