...
with lib;
let
  cfg = config.services.databases.postgres;
...
in
{
  ...
  options.services.databases.postgres = {
    enable = mkEnableOption "Enable postgres database.";
    package = mkOption {
  ...
  config = mkIf cfg.enable {
    ...
    services.postgresql = {
      enable = true;
      package = cfg.package;
      dataDir = mkForce "${cfg.dataDir}/${cfg.package.version}";
      enableTCPIP = true;
      settings = mkForce {
        port = mkForce cfg.port;
        listen_addresses = "*";
      };
      initdbArgs = mkDefault [ "--pgdata=${cfg.dataDir}/${cfg.package.version}" "--auth=trust" "--no-locale" "--encoding=UTF8" ];
